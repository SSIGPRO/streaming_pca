...
// Private functions
uint8_t svd_golub_reinsch_compact(FLOAT_T *U, uint16_t row, uint16_t column, FLOAT_T *singular_values, FLOAT_T * dummy_array);
static void Householders_Reduction_to_Bidiagonal_Form_compact(FLOAT_T *U, uint16_t nrows, uint16_t ncols, FLOAT_T *diagonal, FLOAT_T *superdiagonal);
static int Givens_Reduction_to_Diagonal_Form_compact(uint16_t nrows, uint16_t ncols, FLOAT_T *U, FLOAT_T *diagonal, FLOAT_T *superdiagonal);
static void Sort_by_Decreasing_Singular_Values_compact(uint16_t nrows, uint16_t ncols, FLOAT_T *singular_value, FLOAT_T *U);

// Main function
void ext_mat_svd_compact(
    ARM_MATRIX_INSTANCE * pSrcU,     /* m x n input matrix (and output U matrix) */
                ...
                FLOAT_T * p_m_buff)  /* m sized vector buffer */
{
    ...
    FLOAT_T * ps = pDsts;
    svd_golub_reinsch_compact(pU, n, m, ps, p_m_buff);
}
...
uint8_t svd_golub_reinsch_compact(FLOAT_T *U, uint16_t row, uint16_t column, FLOAT_T *singular_values, FLOAT_T * dummy_array) {
    ...
    Householders_Reduction_to_Bidiagonal_Form_compact(U, row, column, singular_values, dummy_array);

    if (Givens_Reduction_to_Diagonal_Form_compact(row, column, U, singular_values, dummy_array) < 0)
        return 0; // Fail

    Sort_by_Decreasing_Singular_Values_compact(row, column, singular_values, U);
...
static void Sort_by_Decreasing_Singular_Values_compact(uint16_t nrows, uint16_t ncols, FLOAT_T *singular_values, FLOAT_T *U) {
    int i, j, max_index;
    FLOAT_T temp;
    FLOAT_T *p1, *p2;
